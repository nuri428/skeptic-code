...
`skeptic-code`는 묻습니다: **"이 코드가 존재해야 하는가? 그리고 존재해야 한다면 — 그 주장은 증거를 버틸 수 있는가?"**

### 왜 쓰는가?

AI 코딩 실패의 상당수는 문법 오류가 아니라 **확신에 찬 가정**입니다.

```text
"Looks good. Tests pass."
```

skeptic-code는 한 단계 더 내려갑니다:

```text
Claim: caller가 안전하게 .strip()을 호출할 수 있다
Evidence: callee 반환형은 Optional[str]
Reproduction: missing ID -> AttributeError
Invariant: 필수 사용자 식별자는 boundary에서 검증되어야 한다
Fix: 최초 위반 지점에서 contract 복구
Regression test: 추가
Independent checker: APPROVE
```

목표는 지적사항을 많이 만드는 게 아니라 **확신에 찬 오판과 투기적 수정의 수를 줄이는 것**입니다.
...
구조화된 두 개의 탐색 패스, 체크리스트 맹점을 막는 자유 탐색 패스 하나, 그리고 적대적 검증 루프:
...
**Pass 1C — Open Hunt**  
체크리스트를 잠시 무시하고 묻습니다: *"18개 항목이 보라고 하지 않은 것 중 여기서 잘못된 건 무엇인가?"*  
동일한 증거 기준을 적용하며, `CLEAN`은 여전히 유효한 결과입니다.

탐색 패스 이후 모든 후보는 증거로 검증된 뒤 판정이 내려진다. grep 결과와 줄 번호 없이는 `[CUT]`도 `[ADD]`도 없다. 그리고 **런타임** HIGH 판정에는 **재현**이 추가로 요구된다 — 가드는 읽어서가 아니라 실제로 발화시켜서 검증한다 (환경 문제로 재현 불가한 HIGH는 `[UNREPRODUCED-HIGH]`로 심각도를 유지하고 차단 여부는 사용자가 결정).
