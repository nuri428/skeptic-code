...
Dry-check asks: "**Should this code exist at all?**"
...
```
/dry-check              # Auto-detect scope
/dry-check quick        # Top-5 findings, ~2 min
/dry-check deep         # Full line-level audit, ~10 min
/dry-check <path>       # Specific file or directory
```
...
Patterns to hunt immediately. Each one earns a finding.

1. **Helper called once** — extract that's only used at one callsite → inline it
2. **Interface with one implementation** — "in case we add more" → delete the interface
3. **`except: pass` / `catch (_) {}`** — the lie. Remove it or handle it.
4. **`or []` / `?? []` on a required field** — hides the bug upstream
...
Before marking anything `[CUT]`:

```yaml
verification:
  read_full_file: true      # not just the suspicious line — the whole file
  grep_all_callers: true    # confirm usage count across the codebase
  check_scope: true         # is this in tests/, __main__, or dev-only?
  check_comment: true       # is there an explanation for why it exists?

rule: ALL must be true to cut.
...
cut:
  ...
  evidence: "grep shows 1 caller (line 89). No independent test. No comment."
```

If you cannot fill `evidence` with a grep result and line numbers → it's not a `[CUT]`.
...
```bash
wc -l <files>                           # size
grep -rn "def \|class \|function " .    # all symbols defined
grep -rn "<symbol>" .                   # all callers
```

### Step 2: Pass 1 — Hunt

Go through the Wanted List. For each file, note every pattern that matches.
Write down: what, where (file:line), which tag.

### Step 3: Pass 2 — Verify

For each candidate, run the verification checklist.
Read ±30 lines around the finding.
If it passes all four checks → `[CUT]`.
If any check fails → `[QUESTION]` or `[FALSE_ALARM]`.
...
dry_check:
  ...
  findings:
    - id: DC-001
      ...
      evidence: "grep: 1 caller at file.py:89. 0 tests."
      before: |
...
**BLOCKING** — present findings before any edits.

```python
AskUserQuestion(
    question=f"Dry-check: {cut} CUT, {question} QUESTION\n\nBiggest wins: {top_findings}\n\nProceed?",
    options=[
...
| Severity | Examples |
|----------|---------|
| HIGH | Silent failure on hot path, 100+ lines of scope creep |
| MEDIUM | Unused abstraction, helper called once, config ghost |
| LOW | Zombie import, stale comment, defensive null check |

## Example

```
User: /dry-check src/web/router.py
...
Pass 2 — Verifying each:
  ...
  DC-001 [LIAR]: save_config() collects errors[] but always returns HTTP 200
    grep: errors list built but status never set. No test for partial failure.
    Verdict: CUT

  DC-002 [PROPHET]: _MAX_TTL_BYTES constant at module level
...
Dry-check complete: 1 CUT, 1 QUESTION, 1 false alarm.
